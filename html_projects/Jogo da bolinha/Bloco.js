...
class Bloco {
    constructor(ctx,bola,posicaoX,posicaoY,cor) {
        ...
        //* Tamanho do bloco na imagem, Para recorte
        this.larguraSprite = imagemBlocos.width / 2
        this.alturaSprite = imagemBlocos.height / 4

        //* Tamanho do bloco na tela
        this.largura = ctx.canvas.width / 10
        this.altura = ctx.canvas.height / 30
        ...
        //* Tamanho da divisão da tela para desenhar os blocos
    ...
    resize() {
        //* Aqui eu mudo o tamanho que o bloco ocupa
        this.tamanhoLinha = ctx.canvas.width / 10
        this.tamanhoColuna = ctx.canvas.height / 30

        //* Aqui preencho o espaço que separei com o bloco, com exatamente sua altura e largura
        this.largura = ctx.canvas.width / 10
        this.altura = ctx.canvas.height / 30
        //* Aqui determino o X e Y de acordo com o tamanho da tela
        this.x = this.posicaoX * this.tamanhoLinha
        this.y = this.posicaoY * this.tamanhoColuna
    }
    ...
    desenhar() {
        ...
        //? Batida na lateral direita Superior
            ...
            (this.vivo) &&
            (this.bola.x <= this.x + this.largura) &&
            (this.bola.y + this.bola.altura >= this.y && this.bola.y + this.bola.altura <= this.y + this.altura) &&
            (this.bola.x + this.bola.largura >= this.x)) {
                if(!superbola) {
                    this.bola.dirX = 1
                    this.bola.dirY = -1
